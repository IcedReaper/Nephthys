    ...
    private void function load() {
        if(variables.referenceId != null && variables.referenceId != 0) {
            ...
            if(qGetReference.getRecordCount() == 1) {
                ...
                variables.creator      = createObject("component", "API.modules.com.Nephthys.user.user").init(qGetReference.creatorUserId[1]);
                variables.creationDate = qGetReference.creationDate[1];
                variables.lastEditor   = createObject("component", "API.modules.com.Nephthys.user.user").init(qGetReference.lastEditorUserId[1]);
